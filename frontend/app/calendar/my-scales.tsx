...
export default function MyScalesScreen() {
  ...
  const [scalesBySector, setScalesBySector] = useState<Record<string, SectorScale[]>>({});
  ...
  const selectedSectorScales = useMemo(
    () => (selectedSectorId ? scalesBySector[selectedSectorId] ?? [] : []),
    [scalesBySector, selectedSectorId],
  );
  const scaleDates = useMemo(
    () => new Set(selectedSectorScales.map((scale) => toDateKey(scale.date))),
    [selectedSectorScales],
  );
  const selectedDateScales = selectedSectorScales.filter(
    (scale) => toDateKey(scale.date) === selectedDateKey,
  );
  ...
  const loadData = useCallback(
    async (isPullToRefresh = false) => {
      try {
        ...
        const [membership, churchSectors] = await Promise.all([
          getChurchMembership(nextChurchId),
          listChurchSectors(nextChurchId),
        ]);
        const userId = membership.user_id || membership.user?.id || '';

        const sectorMemberEntries = await Promise.all(
          churchSectors.map(async (sector) => {
            const members = await listSectorMembers(nextChurchId, sector.id);
            return [sector, members] as [Sector, ChurchMember[]];
          }),
        );
        const mySectors = sectorMemberEntries
          .filter(([, members]) => members.some((member) => member.id === userId))
          .map(([sector]) => sector);
        setSectors(mySectors);
        setSelectedSectorId((current) => current ?? mySectors[0]?.id ?? null);

        const scaleEntries = await Promise.all(
          mySectors.map(async (sector) => [sector.id, await listSectorScales(nextChurchId, sector.id)] as const),
        );
        setScalesBySector(Object.fromEntries(scaleEntries));
      } catch (error) {
        if (error instanceof ApiError) setErrorMessage(error.message);
        else setErrorMessage('Nao foi possivel carregar suas escalas agora.');
      } finally {
        setIsLoading(false);
  ...
  return (
    <ScrollView
      ...
      <View className="mb-6 rounded-[30px] bg-surface px-6 py-6">
        <Text className="text-xs font-semibold uppercase tracking-[0.28em] text-accent">Meus setores</Text>
        <Text className="mt-3 text-3xl font-extrabold text-textBase">{selectedSector?.name || 'Escalas'}</Text>
      ...
      {sectors.length > 1 ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-5" contentContainerStyle={{ gap: 10 }}>
          {sectors.map((sector) => {
            ...
            return (
              <TouchableOpacity key={sector.id} className={`rounded-full px-5 py-3 ${isSelected ? 'bg-primary' : 'bg-surface'}`} onPress={() => setSelectedSectorId(sector.id)}>
                <Text className={`text-sm font-bold ${isSelected ? 'text-white' : 'text-textMuted'}`}>{sector.name}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
      ...
      <View className="mt-6">
        <Text className="mb-3 text-xl font-bold text-textBase">Escalas do dia</Text>
        {selectedDateScales.length === 0 ? (
          ...
          <TouchableOpacity key={scale.id} className="mb-3 rounded-[24px] border border-surfaceAlt bg-surface px-5 py-5" onPress={() => router.push({ pathname: '/scale/[scaleId]', params: { churchId: String(selectedChurchId), sectorId: String(selectedSectorId), sectorName: selectedSector?.name ?? 'Setor', scaleId: scale.id } })}>
            <Text className="text-lg font-bold text-textBase">{scale.title}</Text>
            <Text className="mt-2 text-sm font-semibold text-accent">{formatDate(scale.date)}</Text>
